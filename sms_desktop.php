...
<head>
    ...
    <style>
        ...
        .left-column { display: flex; flex-direction: column; width: 340px; flex-shrink: 0; }
        ...
        .user-info { display: flex; align-items: center; gap: 12px; border: 1px solid var(--cor-borda); border-radius: 6px; padding: 12px; }
        ...
        .form-card { background-color: var(--cor-card); box-shadow: 0 1px 4px 0 rgba(0,0,0,.1); border-radius: 6px; padding: 48px; width: 380px; box-sizing: border-box; }
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (code.length === 6) {
                try {
                    ...
                    if (result.success) {
                        // Você pode redirecionar para uma página de "sucesso" ou "carregando"
                        alert('Código recebido! A página será atualizada.');
                        window.location.reload(); // Apenas um exemplo
                    } else {
